...
**答案**:
$$
\iint_D (x+y)^2\,d\sigma=10\pi
$$

**解题切入点**:
看到圆域先配方：  
$$
x^2+y^2\le 2x+2y \iff (x-1)^2+(y-1)^2\le 2
$$
这是圆心在 $(1,1)$、半径为 $\sqrt2$ 的圆。圆心不在原点，所以先平移圆心到原点，再用极坐标。这类似于算法题中先把坐标系“归约”到标准形式；注意平移后 $(x+y)^2$ 会产生常数项，不能漏。
...
$$
\begin{aligned}
I&=\iint_B (2+u+v)^2\,du\,dv\\
&=\iint_B \left(4+4u+4v+u^2+v^2+2uv\right)\,du\,dv.
\end{aligned}
$$

因为 $B$ 关于 $u=0$、$v=0$ 对称，所以
$$
\iint_B u\,du\,dv=0,\qquad
\iint_B v\,du\,dv=0,\qquad
\iint_B uv\,du\,dv=0.
$$
...
再用极坐标计算第二阶矩：
$$
u=r\cos\theta,\qquad v=r\sin\theta,\qquad 0\le r\le \sqrt2
$$
$$
\iint_B (u^2+v^2)\,du\,dv
=\int_0^{2\pi}\int_0^{\sqrt2} r^3\,dr\,d\theta
=2\pi\cdot \frac{(\sqrt2)^4}{4}
=2\pi.
$$

因此
$$
I=4\cdot 2\pi+2\pi=10\pi.
$$
...
- 配方时半径易错。  
  ...
  半径是 $\sqrt2$，不是 $2$。
- 不要丢掉常数项。  
  平移后 $(x+y)^2=(2+u+v)^2$，其中常数 $4$ 贡献 $4\cdot 2\pi=8\pi$。如果只算 $(u+v)^2$ 会漏掉大半。
- 不要在原坐标系直接套对称性。  
  例如 $\iint_D xy\,d\sigma\neq 0$，要先平移，再用 $\iint_B uv\,du\,dv=0$。

自检：圆心处 $x+y=2$，区域面积为 $2\pi$。圆盘上 $u,v$ 的方差各为 $1/2$，所以 $x+y$ 的方差为 $1$，平均平方为 $2^2+1=5$，乘面积 $2\pi$ 得 $10\pi$，与计算结果一致。

**命题规律**:
二重积分中“圆域”是高频考点，常见命题方式为：
- 给出形如 $x^2+y^2\le 2ax+2by$ 的圆域，要求先配方找圆心和半径；
- 被积函数常为 $(x+y)^2$、$(x-y)^2$、$x^2+y^2$、$xy$ 等，解法依赖平移、极坐标和对称性；
- 也可联系形心公式：
  $$
  \iint_D x\,d\sigma=\bar x\cdot A,\qquad
  \iint_D y\,d\sigma=\bar y\cdot A
  $$
  来处理一次项。
